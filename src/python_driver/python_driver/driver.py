...
from python_driver_interfaces.srv import ChangeMode, ResetAxis
...
class ServoDriver(Node):
    def __init__(self):
        ...
        self.servos = [ MksServo(self.bus, self.notifier, 1), MksServo(self.bus, self.notifier, 2) ]
        self.mode = 0  # mode: 0 - standby, 1 - position, 2 - speed
...
# MISC
    ...
    def servo_init(self, servo):
        servo.set_work_mode(MksServo.WorkMode.SrClose)
        servo.set_subdivisions(64)
        servo.set_working_current(3200)
...
# SERVO COMMANDS
    ...
    def servo_speed_mode(self, servo, speed):
        ...
        servo.run_motor_in_speed_mode(Direction.CCW if speed_rpm > 0 else Direction.CW, min(abs(speed_rpm),3000), 10)
